...
def main():
    ...
    parser.add_argument('--log_name', type=str, 
                        default='EX-2025-05-07-encoder_decoder_ft_encoder', help='实验名称')
    ...
    parser.add_argument('--batch_size', type=int, default=7, help='批次大小')
    parser.add_argument('--epochs', type=int, default=300, help='训练轮数')
    parser.add_argument('--num_workers', type=int, default=2, help='数据加载线程数')
    parser.add_argument('--encoder_name', type=str, default='timm-regnety_064', help='编码器名称')
    parser.add_argument('--encoder_weights', type=str, default='imagenet', help='编码器预训练权重')
    parser.add_argument('--pretrained', type=str, 
                        default=r'D:\2025\Petal_diseasa-master\logs\EX-2025-04-25-step3_regnety_064_panet\version_2\checkpoints\epoch=99-step=31500.ckpt',
                         help='预训练权重路径')
    parser.add_argument('--decoder_name', type=str, default='pan', help='解码器名称')
    parser.add_argument('--freeze_layers', default=[], help='冻结层的名称')
    ...
    save_log_dir = args.log_name.replace('encoder_decoder', 
                                args.encoder_name+'-'+args.decoder_name)
    logger = TensorBoardLogger(save_dir=args.log_dir, name=save_log_dir)
    ...
    trainer = pl.Trainer(
        ...
        log_every_n_steps=1,
        logger=logger,
        devices=[int(args.device)],  # 使用双GPU模式
        accelerator='gpu',
