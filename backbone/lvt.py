# cited: Lite Vision Transformer with Enhanced Self-Attention
...
class CSA(nn.Module):
    ...
    def forward(self, x, v=None):
        ...
        attn = self.pool(x.permute(0, 3, 1, 2)).permute(0, 2, 3, 1) # h/s, w/s, c
        attn = self.attn(attn).reshape(
            B, h * w, self.num_heads, self.kernel_size * self.kernel_size, 
            self.kernel_size * self.kernel_size).permute(0, 2, 1, 3, 4) # B,H,N,kxk,kxk; 
        # h/s, w/s, c -> h/s,w/s,k**4xn ->  B, (h/s*w/s), n, kxk, kxk
        ...
        ).permute(0,3,2,1).reshape(B*h*w, 
                                self.kernel_size*self.kernel_size*self.out_dim, 1, 1)
        # B,C,H,W->
        v = self.weight(v)
        ...
        if self.use_rasa:
            if self.rasa_cfg.atrous_rates is not None:
                self.ds = ds_conv2d(
                    ...
                    act_layer=self.rasa_cfg.act_layer, 
                    init=self.rasa_cfg.init_method,
                )
...
class lite_vision_transformer(nn.Module):
    ...
    def __init__(self, layers, in_chans=3, num_classes=1000, patch_size=4,
        ...
        self.layers        = layers # [2,2,2,2]
        self.num_classes   = num_classes
        self.sa_layers     = sa_layers
        self.rasa_cfg      = rasa_cfg # I am here including 1) atrous_rates; 2) act_layer; 3) init_methods
        self.with_cls_head = with_cls_head # set false for downstream tasks

        # for detectron2 architecture.
        self._out_feature_channels = {}
        self._out_feature_strides  = {}
        self._out_features = []
   
        network = []
        for stage_idx in range(len(layers)): # [2,2,2,2] = 4
            name = f's_{stage_idx+1}'
            ...
            self._out_feature_channels[name] = embed_dims[stage_idx] 
            s = 2**(stage_idx + 2)
    ...
    def forward(self, x):
        if self.with_cls_head:
            ...
            outs = {}
            for idx, stage in enumerate(self.backbone):
                stage_name = f's_{idx+1}'
                x = stage(x)
                x = self.downstream_norms[idx](x)
                outs[stage_name] = x.permute(0,3,1,2).contiguous()
            return outs
        
@BACKBONE_REGISTRY.register()
class lvt2mutilout(lite_vision_transformer, Backbone):
    def __init__(self, cfg, **kwargs):
        rasa_cfg      = self._get_rasa_cfg(cfg) 
        layers        = cfg.MODEL.BACKBONE.layers             # [2,2,2,2]
        patch_size    = cfg.MODEL.BACKBONE.patch_size         # 4
        embed_dims    = cfg.MODEL.BACKBONE.embed_dims         # [64,64,160,256]
        num_heads     = cfg.MODEL.BACKBONE.num_heads          # [2,2,5,8]
        mlp_ratios    = cfg.MODEL.BACKBONE.mlp_ratios         # [4,8,4,4]
        mlp_depconv   = cfg.MODEL.BACKBONE.mlp_depconv        # [False, True, True, True]
        sr_ratios     = cfg.MODEL.BACKBONE.sr_ratios          # [8,4,2,1]
        sa_layers     = cfg.MODEL.BACKBONE.sa_layers          # ['csa', 'rasa', 'rasa', 'rasa']
        with_cls_head = cfg.MODEL.BACKBONE.with_cls_head      # False
        super().__init__(
            layers      = layers,
    ...
    def _get_rasa_cfg(self,cfg):
        if cfg.MODEL.BACKBONE.rasa_cfg is None:
            return None
        else:
            rasa_cfg = get_rasa_cfg(cfg)
            return rasa_cfg
...
class get_rasa_cfg(object):
    def __init__(self, cfg=None):
        self.atrous_rates = cfg.MODEL.BACKBONE.rasa_cfg.atrous_rates
        self.act_layer    = cfg.MODEL.BACKBONE.rasa_cfg.act_layer
        self.init_method  = cfg.MODEL.BACKBONE.rasa_cfg.init_methods
        self.r_num        = cfg.MODEL.BACKBONE.rasa_cfg.r_num
