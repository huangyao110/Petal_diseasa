...
from utils.inference import InferenceRoseDisease
...
class ProcessingWorker(QThread):
    ...
    def _extract_id_from_filename(self, filename):
        """Extract ID 81 from filename crop_81.jpg"""
        try:
    ...
    def _create_annotated_image(self, original_path, results, result_dir, base_name):
        """Create annotated image with masks and IDs"""
        try:
            ...
            for r in results:
                ...
                # Overlay masks (using configured colors)
...
# ==========================================
# 4. UI Components (HomeTab, ConfigTab)
# ==========================================
...
class HomeTab(QWidget):
    ...
    def _init_logo(self):
        """Initialize bottom-left Logo"""
        self.logo_label = QLabel(self)
        # Allow mouse events to pass through Logo
            ...
            if not pix.isNull():
                ...
                for y in range(image.height()):
                    for x in range(image.width()):
                        ...
                        pixel.setAlpha(250) # Set transparency (0-255, 120 is semi-transparent)
        ...
        # Place logo at bottom-left
        self.logo_label.move(20, self.height() - 190)
        # Bring to top layer
        self.logo_label.raise_()

    def resizeEvent(self, event):
        """When window size changes, keep logo in bottom-left"""
        super().resizeEvent(event)
        
        # Bottom-left logo
        if hasattr(self, 'logo_label'):
            self.logo_label.move(20, self.height() - 220)
            # Keep logo on top layer after resize
            self.logo_label.raise_()
    ...
    def _init_ui(self):
        ...
        # --- Right: Result Display Area (with view switching) ---
        ...
        # Stacked layout: 0=image, 1=table
        self.stack = QStackedWidget()
        
        # Page 0: Image scroll area
    ...
    def display_results(self, results):
        ...
        if self.is_batch_mode:
            ...
            # 2. Populate image preview (enhanced: large image + fullscreen button)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.resize(1280, 960) # Default large window
        
        if os.path.exists(LOGO_FILE):
            self.setWindowIcon(QIcon(LOGO_FILE))
        
        # Fix: Explicitly specify utf-8 when loading style sheet
